...
class JsonStorage {
    ...
    _blobServerPromise;
    constructor(disablePersistence = false) {
    ...
    checkHealth() {
        return Promise.reject("Should not be running JSON storage in production");
    }
    addAccount(account) {
    ...
    getUserFromAccessKey(accessKey) {
        return this.getAccountIdFromAccessKey(accessKey).then((accountId) => {
            return this.getAccount(accountId);
        });
    }
    getUserFromAccessToken(accessToken) {
        return this.getAccountIdFromAccessKey(accessToken).then((accountId) => {
            return this.getAccount(accountId);
        }).catch(error => {
            throw error;
        });
    ...
    getPackageHistoryFromDeploymentKey(deploymentKey) {
        ...
        return Promise.resolve(clone(this.deployments[deploymentId].packageHistory));
    }
