...
const aws_storage_1 = require("./aws-storage");
// Define the Sequelize connection
const sequelize = new sequelize_1.Sequelize("codepushdb", "root", "root", {
    host: "localhost",
    dialect: "mysql",
});
// Seed data
...
async function seed() {
    try {
        // Initialize models
        const models = (0, aws_storage_1.createModelss)(sequelize);
        // Sync database
        ...
        for (const account of exports.seedData.accounts) {
            await models.Account.upsert(account);
        }
        console.log("Seeding tenants...");
        for (const tenant of exports.seedData.tenants) {
            await models.Tenant.upsert(tenant);
        }
        console.log("Seeding apps...");
        for (const app of exports.seedData.apps) {
        ...
        await models.Collaborator.destroy({
            where: {
                appId: exports.seedData.apps.map(app => app.id)
            }
        });
        await models.Collaborator.bulkCreate(exports.seedData.collaborators);
        console.log("Seeding deployments...");
        for (const deployment of exports.seedData.deployments) {
        ...
        for (const pkg of exports.seedData.packages) {
        ...
        for (const deployment of exports.seedData.deployments) {
            if (deployment.packageId) {
                await models.Deployment.update({ packageId: deployment.packageId }, { where: { id: deployment.id } });
            }
        }
        console.log("Seeding access keys...");
        for (const accessKey of exports.seedData.accessKeys) {
