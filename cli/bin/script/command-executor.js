...
const configFilePath = path.join(process.env.LOCALAPPDATA || process.env.HOME, ".dota.config");
...
function orgList(command) {
    throwForInvalidOutputFormat(command.format);
    let orgs;
    return exports.sdk.getTenants().then((retrievedOrgs) => {
    ...
    return Q(null).then(() => {
        switch (command.type) {
            ...
            default:
                ...
                if (!connectionInfo) {
                    throw new Error("You are not currently logged in. Run the 'dota login' command to authenticate with the Dota server.");
                }
                ...
                if (command.appName) {
                    ...
                    if (parsedName.ownerName) {
                        command.appName = parsedName.appName;
                        exports.sdk.passedOrgName = parsedName.ownerName;
                    }
                }
                else if (command.org && command.org.length > 0) {
                    const arg = command.org;
                    exports.sdk.passedOrgName = arg;
                }
                break;
        }
        switch (command.type) {
...
function printOrgList(format, orgs) {
    if (format === "json") {
        printJson(orgs);
    }
    else if (format === "table") {
...
const releaseReact = (command) => {
    ...
    const outputFolder = command.outputDir || path.join(os.tmpdir(), "Dota");
...
function serializeConnectionInfo(accessKey, preserveAccessKeyOnLogout, customServerUrl) {
    ...
    (0, exports.log)(`\r\nSuccessfully logged-in. Your session file was written to ${chalk.cyan(configFilePath)}. You can run the ${chalk.cyan("dota logout")} command at any time to delete this file and terminate your session.\r\n`);
}
